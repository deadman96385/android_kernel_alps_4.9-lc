...
#define INMSG() WD_DEV_DBG(_DEV(tee), "%s: >\n", __func__)
#define OUTMSG(val) WD_DEV_DBG(_DEV(tee), "%s: < %ld\n", __func__, (long)val)
#define OUTMSGX(val) WD_DEV_DBG(_DEV(tee), "%s: < %08x\n",\
...
struct tee_shm *tee_shm_alloc(struct tee *tee, size_t size, uint32_t flags)
{
	...
	WD_DEV_DBG(_DEV(tee),
		"%s: shm=%p, paddr=%p,s=%d/%d app=\"%s\" pid=%d\n",
...
static struct sg_table *_tee_shm_dma_buf_map_dma_buf(struct dma_buf_attachment
						     *attach,
...
static void _tee_shm_dma_buf_release(struct dma_buf *dmabuf)
{
	...
	WD_DEV_DBG(_DEV(ctx->tee),
		"%s: shm=%p, paddr=%p,s=%d/%d app=\"%s\" pid=%d\n", __func__,
		shm, (void *)shm->paddr, (int)shm->size_req,
		(int)shm->size_alloc, current->comm, current->pid);
				 ...
				 struct vm_area_struct *vma)
{
	...
	WD_DEV_DBG(_DEV(shm->ctx->tee),
		"%s: map the shm (p@=%p,s=%dKiB) => %x\n",
...
static void *_tee_shm_dma_buf_kmap(struct dma_buf *db, unsigned long pgnum)
{
	...
	WD_DEV_DBG(_DEV(shm->ctx->tee),
		"%s: kmap the shm (p@=%p, v@=%p, s=%zdKiB)\n", __func__,
		(void *)shm->paddr, (void *)shm->kaddr, shm->size_alloc / 1024);
	/*
...
static void _tee_shm_dma_buf_kunmap(struct dma_buf *db, unsigned long pfn,
				    void *kaddr)
{
			  ...
			  unsigned int flags, size_t size, int offset)
{
	...
	WD_DEV_DBG(_DEV(tee), "%s: > fd=%d flags=%08x\n", __func__, fd, flags);
	...
	WD_DEV_DBG(_DEV(tee), "fd=%d @p=%p is_tee=%d db=%p\n", fd,
...
static unsigned int tee_shm_get_phy_from_kla(struct mm_struct *mm,
					     unsigned int kla)
{
	pgd_t *pgd;
	pud_t *pud;
...
static int tee_shm_va_get(struct tee_context *ctx, struct tee_shm *shm,
			  void *buffer, unsigned int flags, size_t size,
			  int offset)
{
	int ret = 0;
	...
	if (vma) {
		...
		if (vma->vm_end - vma->vm_start - offset_total < size) {
			dev_err(_DEV(tee),
				"%s %p:%x not big enough: %lx - %d < %x\n",
				__func__, buffer, shm->paddr,
				vma->vm_end - vma->vm_start, offset_total,
				size);
			shm->paddr = 0;
			ret = -EINVAL;
			goto out;
		...
		if (!phys_base) {
			...
			dev_err(_DEV(tee),
				"%s can't get physical address for %p\n",
				__func__, buffer);
			goto err;
		}
		...
		size_t size, int offset)
{
	...
	if (c_shm->flags & TEEC_MEM_KAPI) {
		...
		if (kc_shm->size_alloc < size + offset) {
			dev_err(_DEV(tee),
				"kapi buff too small %zd < %zd + %d\n",
				kc_shm->size_alloc, size, offset);
			ret = -EINVAL;
			goto err;
		}

		WD_DEV_DBG(_DEV(tee), "fd=%d @p=%p\n",
		...
		WD_DEV_DBG(_DEV(tee), "null buffer, pass 'as is'\n");
	} else {
#ifdef VA_GET_ENABLED
...
{
	struct tee *tee __attribute__((unused)) = ctx->tee;

	WD_DEV_DBG(_DEV(tee), "%s: > shm=%p flags=%08x paddr=%p\n",
