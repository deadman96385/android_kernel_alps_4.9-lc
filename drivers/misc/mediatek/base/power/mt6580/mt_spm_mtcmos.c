...
int spm_mtcmos_ctrl_cpu0(int state, int chkWfiBeforePdn)
{
	...
	spm_write_trace(SPM_POWERON_CONFIG_SET, (SPM_PROJECT_CODE << 16) | (1U << 0));
	...
	if (state == STA_POWER_DOWN) {
		...
		spm_write_trace(SPM_CA7_CPU0_PWR_CON,
		...
		spm_write_trace(SPM_CA7_CPU0_L1_PDN,
...
int spm_mtcmos_ctrl_cpu1(int state, int chkWfiBeforePdn)
{
	...
	if (state == STA_POWER_DOWN) {
		...
		spm_write_trace(SPM_CA7_CPU1_PWR_CON,
		...
		spm_write_trace(SPM_CA7_CPU1_L1_PDN,
...
int spm_mtcmos_ctrl_cpu2(int state, int chkWfiBeforePdn)
{
	...
	if (state == STA_POWER_DOWN) {
		...
		spm_write_trace(SPM_CA7_CPU2_PWR_CON,
		...
		spm_write_trace(SPM_CA7_CPU2_L1_PDN,
...
int spm_mtcmos_ctrl_cpu3(int state, int chkWfiBeforePdn)
{
	...
	if (state == STA_POWER_DOWN) {
		...
		spm_write_trace(SPM_CA7_CPU3_PWR_CON,
		...
		spm_write_trace(SPM_CA7_CPU3_L1_PDN,
...
int spm_mtcmos_ctrl_cpusys0(int state, int chkWfiBeforePdn)
{
	...
	if (state == STA_POWER_DOWN) {
		...
		spm_write_trace(SPM_CA7_CPUTOP_PWR_CON,
		...
		spm_write_trace(SPM_CA7_CPUTOP_L2_PDN,
...
int spm_mtcmos_ctrl_isp(int state)
{
	...
	if (state == STA_POWER_DOWN) {
		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) | SRAM_PDN);
		while ((spm_read(SPM_ISP_PWR_CON) & ISP_SRAM_ACK) != ISP_SRAM_ACK)
			;

		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) | PWR_ISO);

		val = spm_read(SPM_ISP_PWR_CON);
		val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
		spm_write_trace(SPM_ISP_PWR_CON, val);

		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) & ~(PWR_ON | PWR_ON_2ND));

		while ((spm_read(SPM_PWR_STATUS) & ISP_PWR_STA_MASK) ||
			(spm_read(SPM_PWR_STATUS_2ND) & ISP_PWR_STA_MASK))
			;
	} else {    /* STA_POWER_ON */
		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) | PWR_ON);
		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) | PWR_ON_2ND);

		while (!(spm_read(SPM_PWR_STATUS) & ISP_PWR_STA_MASK) ||
			!(spm_read(SPM_PWR_STATUS_2ND) & ISP_PWR_STA_MASK))
			;

		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) & ~PWR_CLK_DIS);
		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) & ~PWR_ISO);
		spm_write_trace(SPM_ISP_PWR_CON, spm_read(SPM_ISP_PWR_CON) | PWR_RST_B);
...
int spm_mtcmos_ctrl_disp(int state)
{
	...
	if (state == STA_POWER_DOWN) {
		spm_write_trace(TOPAXI_PROT_EN, spm_read(TOPAXI_PROT_EN) | DISP_PROT_MASK);
		...
		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) | SRAM_PDN);
		...
		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) | PWR_ISO);

		val = spm_read(SPM_DIS_PWR_CON);
		val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
		spm_write_trace(SPM_DIS_PWR_CON, val);

		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) & ~(PWR_ON | PWR_ON_2ND));

		while ((spm_read(SPM_PWR_STATUS) & DIS_PWR_STA_MASK)
			|| (spm_read(SPM_PWR_STATUS_2ND) & DIS_PWR_STA_MASK))
			;
	} else {    /* STA_POWER_ON */
		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) | PWR_ON);
		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) | PWR_ON_2ND);

		while (!(spm_read(SPM_PWR_STATUS) & DIS_PWR_STA_MASK)
			|| !(spm_read(SPM_PWR_STATUS_2ND) & DIS_PWR_STA_MASK))
			;

		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) & ~PWR_CLK_DIS);
		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) & ~PWR_ISO);
		spm_write_trace(SPM_DIS_PWR_CON, spm_read(SPM_DIS_PWR_CON) | PWR_RST_B);
...
int spm_mtcmos_ctrl_mfg(int state)
{
	...
	spm_mtcmos_noncpu_lock(&flags);
	if (state == STA_POWER_DOWN) {
		spm_write_trace(TOPAXI_PROT_EN, spm_read(TOPAXI_PROT_EN) | MFG_ASYNC_PROT_MASK);
		while ((spm_read(TOPAXI_PROT_STA1) & MFG_ASYNC_PROT_MASK) != MFG_ASYNC_PROT_MASK) {
			count++;
			if (count > 1000)
				break;
		}
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) | MFG_SRAM_PDN);
		count = 0;
		...
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) | PWR_ISO);
		val = spm_read(SPM_MFG_PWR_CON);
		val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
		spm_write_trace(SPM_MFG_PWR_CON, val);
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) & ~(PWR_ON | PWR_ON_2ND));
		count = 0;
		...
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) | PWR_ON);
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) | PWR_ON_2ND);
		while (!(spm_read(SPM_PWR_STATUS) & MFG_PWR_STA_MASK) ||
			!(spm_read(SPM_PWR_STATUS_2ND) & MFG_PWR_STA_MASK)) {
			count++;
			if (count > 1000 && count < 1005) {
		...
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) & ~PWR_CLK_DIS);
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) & ~PWR_ISO);
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) | PWR_RST_B);
		spm_write_trace(SPM_MFG_PWR_CON, spm_read(SPM_MFG_PWR_CON) & ~MFG_SRAM_PDN);
		count = 0;
...
int spm_mtcmos_ctrl_mdsys1(int state)
{
	...
	if (state == STA_POWER_DOWN) {
		spm_write_trace(TOPAXI_PROT_EN, spm_read(TOPAXI_PROT_EN) | MD1_PROT_MASK);
		...
		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) | MD_SRAM_PDN);

		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) | PWR_ISO);

		val = spm_read(SPM_MD_PWR_CON);
		val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
		spm_write_trace(SPM_MD_PWR_CON, val);

		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) & ~(PWR_ON | PWR_ON_2ND));
		...
		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) | PWR_ON);
		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) | PWR_ON_2ND);

		while (!(spm_read(SPM_PWR_STATUS) & MD1_PWR_STA_MASK) ||
			!(spm_read(SPM_PWR_STATUS_2ND) & MD1_PWR_STA_MASK))
			;

		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) & ~PWR_CLK_DIS);
		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) & ~PWR_ISO);
		spm_write_trace(SPM_MD_PWR_CON, spm_read(SPM_MD_PWR_CON) | PWR_RST_B);
...
int spm_mtcmos_ctrl_connsys(int state)
{
	...
	if (state == STA_POWER_DOWN) {
		spm_write_trace(TOPAXI_PROT_EN, spm_read(TOPAXI_PROT_EN) | CONN_PROT_MASK);
		...
		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) | CONN_SRAM_PDN);

		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) | PWR_ISO);

		val = spm_read(SPM_CONN_PWR_CON);
		val = (val & ~PWR_RST_B) | PWR_CLK_DIS;
		spm_write_trace(SPM_CONN_PWR_CON, val);

		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) & ~(PWR_ON | PWR_ON_2ND));

		while ((spm_read(SPM_PWR_STATUS) & CONN_PWR_STA_MASK) ||
			(spm_read(SPM_PWR_STATUS_2ND) & CONN_PWR_STA_MASK))
			;
	} else {
		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) | PWR_ON);
		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) | PWR_ON_2ND);

		while (!(spm_read(SPM_PWR_STATUS) & CONN_PWR_STA_MASK) ||
			!(spm_read(SPM_PWR_STATUS_2ND) & CONN_PWR_STA_MASK))
			;

		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) & ~PWR_CLK_DIS);
		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) & ~PWR_ISO);
		spm_write_trace(SPM_CONN_PWR_CON, spm_read(SPM_CONN_PWR_CON) | PWR_RST_B);
